...
method_name = str(sp.CONFIG_CODE)+"_LFU"     
...
class Controller(object):
    ...
    def provisioning(self):
        while True:
            for edge_server in self.edge_servers :
                edge_server.LFU_provisioning()  
            yield self.env.timeout(sp.SCHEDULER_INTERVAL*3+2)        
    ...
    def allocate_resources(self, requests):
        ...
        for request in requests:
            ...
            accessible_servers = user.accessible_servers
            accessible_servers.reverse()
            ...
            for accessible_server in accessible_servers:
                ...
                if len(available_resource) > 0:
                    ...
                    res = self.edge_servers[accessible_server["server_id"]].allocate_container(request['user_id'],request['task_name'],request['instance_name'],
                    ...
                    prov_list[accessible_server["server_id"]].append(request)

        for edge_server in self.edge_servers :
            edge_server.LFU_rank()
            edge_server.request_list += prov_list[edge_server.id]
