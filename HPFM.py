...
method_name = str(sp.CONFIG_CODE)+"_HPFM"     
...
class Controller(object):
    ...
    def allocate_resources(self, requests):
        ...
        requests.sort(key=lambda x: x['token'], reverse=True)
        for request in requests:
            ...
            if request["status"] != 'allocated':
               for accessible_server in user.accessible_servers:
                    container_id = self.edge_servers[accessible_server["server_id"]].release_LRU_container(request['function_name'])
    ...
    def new_user(self,id) :
        return True
        jitter = int(sp.USER_NUM_JITTER*self.default_user_num)
        num_of_users = self.default_user_num + random.randint(-1*jitter,jitter)
        active_users = sum(1 for u in self.users if u.status!='done')
        if active_users < num_of_users :
            for i in range(num_of_users-active_users) :
                self.users.append(ef.User(self.env,len(self.users)))
